from .database import db, DataValue, Framework, Company, Indicator, Metric, FrameworkMetric, MetricIndicator
...
def get_company_frameworks(company_id):
    result = (
        db.session.query(
    ...
    cur_metric = {}
    for res in result:
        print(f"{res.framework_id} {res.framework_name} {res.metric_id} {res.metric_name} {res.indicator_id} {res.indicator_name}")
